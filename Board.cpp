...
bool Board::availableMove(Move move){
    ...
    auto pieceFrom_index		= getPiece(ptFrom);
    auto pieceTo_index		    = getPiece(ptTo);

	auto pieceFrom = BoardPiece[pieceFrom_index];
	auto pieceTo = BoardPiece[pieceTo_index];
    ...
    do{
		...
		auto cmpPiece = BoardPiece[getPiece(cmpPoint)];
...
void Board::moveChess(Pieces *fromPiece, Point to, Pieces* toPiece) {
...
bool Board::moveable(Point from, Point to) {
	return true;
}
